...
public class FuckingNigger {
    ...
    public float[] middleRotation(final Entity entity, final float currentYaw, final float currentPitch, final boolean random) {
        ...
        final double y = entity.posY + entity.getEyeHeight() - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight());
    ...
    public float[] faceEntityCustom(final Entity entity, float yawSpeed, float pitchSpeed, final float currentYaw, final float currentPitch, final String randomMode, final boolean interpolateRotation, final boolean smartAim, final boolean stopOnTarget, final float randomStrength, final Vec3 best, final boolean throughWalls, final boolean advancedRots, final boolean heuristics, final boolean intave, final boolean bestHitVec) {
        if (smartAim && !throughWalls) {
            ...
            this.y = ePosY - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight());
            ...
            this.y = ey - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight());
        ...
        if (interpolateRotation) {
            yaw = this.interpolateRotation(currentYaw, calcYaw, yawSpeed / NEwMathUtil.nextFloat(170.0f, 180.0f));
            pitch = this.interpolateRotation(currentPitch, calcPitch, pitchSpeed / NEwMathUtil.nextFloat(170.0f, 180.0f));
        }
        else if (heuristics) {
        ...
        switch (randomMode) {
            case "Basic": {
                yaw += (float)(intave ? (NEwMathUtil.nextSecureFloat(1.0, 2.0) * Math.sin(pitch * 3.141592653589793) * randomStrength) : (ThreadLocalRandom.current().nextGaussian() * randomStrength));
                pitch += (float)(intave ? (NEwMathUtil.nextSecureFloat(1.0, 2.0) * Math.sin(yaw * 3.141592653589793) * randomStrength) : (ThreadLocalRandom.current().nextGaussian() * randomStrength));
                break;
            }
            case "OnlyRotation": {
            ...
            case "Doubled": {
                final float random1 = NEwMathUtil.nextSecureFloat(-randomStrength, randomStrength);
                final float random2 = NEwMathUtil.nextSecureFloat(-randomStrength, randomStrength);
                final float random3 = NEwMathUtil.nextSecureFloat(-randomStrength, randomStrength);
                final float random4 = NEwMathUtil.nextSecureFloat(-randomStrength, randomStrength);
                yaw += NEwMathUtil.nextSecureFloat(Math.min(random1, random2), Math.max(random1, random2));
                pitch += NEwMathUtil.nextSecureFloat(Math.min(random3, random4), Math.max(random3, random4));
                break;
            }
        }
        if (advancedRots) {
    ...
    private double[] heuristics(final Entity entity, final double[] xyz) {
        ...
        xyz[0] = MathHelper.clamp_double(xyz[0] + NEwMathUtil.randomSin(), minX, maxX);
        xyz[1] = MathHelper.clamp_double(xyz[1] + NEwMathUtil.randomSin(), minY, maxY);
        xyz[2] = MathHelper.clamp_double(xyz[2] + NEwMathUtil.randomSin(), minZ, maxZ);
        return xyz;
    }
    ...
    public float rotateToYaw(final float yawSpeed, final float currentYaw, final float calcYaw) {
        float yaw = updateRotation(currentYaw, calcYaw, yawSpeed + NEwMathUtil.nextFloat(0.0f, 15.0f));
        final double diffYaw = MathHelper.wrapAngleTo180_float(calcYaw - currentYaw);
        if (-yawSpeed > diffYaw || diffYaw > yawSpeed) {
            yaw += (float)(NEwMathUtil.nextFloat(1.0f, 2.0f) * Math.sin(mc.thePlayer.rotationPitch * 3.141592653589793));
        }
        if (yaw == currentYaw) {
            return currentYaw;
        }
    ...
    public float rotateToYaw(final float yawSpeed, final float[] currentRots, final float calcYaw) {
        float yaw = updateRotation(currentRots[0], calcYaw, yawSpeed + NEwMathUtil.nextFloat(0.0f, 15.0f));
        if (yaw != calcYaw) {
            yaw += (float)(NEwMathUtil.nextFloat(1.0f, 2.0f) * Math.sin(currentRots[1] * 3.141592653589793));
        }
    ...
    public float rotateToPitch(final float pitchSpeed, final float currentPitch, final float calcPitch) {
        float pitch = updateRotation(currentPitch, calcPitch, pitchSpeed + NEwMathUtil.nextFloat(0.0f, 15.0f));
        if (pitch != calcPitch) {
            pitch += (float)(NEwMathUtil.nextFloat(1.0f, 2.0f) * Math.sin(mc.thePlayer.rotationYaw * 3.141592653589793));
        }
        if (mc.gameSettings.mouseSensitivity == 0.5) {
            mc.gameSettings.mouseSensitivity = 0.47887325f;
        }
    ...
    public float rotateToPitch(final float pitchSpeed, final float[] currentRots, final float calcPitch) {
        float pitch = updateRotation(currentRots[1], calcPitch, pitchSpeed + NEwMathUtil.nextFloat(0.0f, 15.0f));
        if (pitch != calcPitch) {
            pitch += (float)(NEwMathUtil.nextFloat(1.0f, 2.0f) * Math.sin(currentRots[0] * 3.141592653589793));
        }
        if (mc.gameSettings.mouseSensitivity == 0.5) {
            mc.gameSettings.mouseSensitivity = 0.47887325f;
        }
    ...
    public float[] backRotate(final float yawSpeed, final float pitchSpeed, final float currentYaw, final float currentPitch, final float calcYaw, final float calcPitch) {
        float yaw = updateRotation(currentYaw, calcYaw + NEwMathUtil.nextFloat(-2.0f, 2.0f), 20.0f + NEwMathUtil.nextFloat(0.0f, 15.0f));
        float pitch = updateRotation(currentPitch, calcPitch + NEwMathUtil.nextFloat(-2.0f, 2.0f), 10.0f + NEwMathUtil.nextFloat(0.0f, 15.0f));
    ...
    public static float[] scaffoldRots(final double bx, final double by, final double bz, final float lastYaw, final float lastPitch, final float yawSpeed, final float pitchSpeed, final boolean random) {
        ...
        final float calcPitch = (float)(-(MathHelper.func_181159_b(y, MathHelper.sqrt_double(x * x + z * z)) * 180.0 / 3.141592653589793));
        float pitch = updateRotation(lastPitch, calcPitch, pitchSpeed + NEwMathUtil.nextFloat(0.0f, 15.0f));
        float yaw = updateRotation(lastYaw, calcYaw, yawSpeed + NEwMathUtil.nextFloat(0.0f, 15.0f));
        if (random) {
    ...
    public float[] advancedScaffoldRots(final BlockPos blockPos, final double[] expandXZ, final float yaw, final float[] lastRots, final float yawSpeed, final float pitchSpeed) {
        ...
        for (float i = pitches.get(0); i < pitches.get(pitches.size() - 1); i += 0.01f) {
            final MovingObjectPosition j = mc.thePlayer.customRayTrace(4.5, 1.0f, yaw, i);
        ...
        if (mc.theWorld.getBlockState(b).getBlock().getMaterial() == Material.air && k != null) {
            final float[] f = this.scaffoldRots(k.hitVec.xCoord, k.getBlockPos().getY() + NEwMathUtil.nextDouble(0.45, 0.55), k.hitVec.zCoord, lastRots[0], lastRots[1], yawSpeed, pitchSpeed, false);
    ...
    private float[] testRots(final float currentYaw, final float currentPitch, float calcYaw, float calcPitch, final Entity entity, final float speedYaw, final float speedPitch, final Vec3 best, final boolean smartAim, final boolean throughWalls) {
        final double radius = NEwMathUtil.nextDouble(0.001, 2.0);
        this.lastAngle = ((this.lastAngle > 360.0) ? 0.0 : (this.lastAngle + NEwMathUtil.nextDouble(-0.4, 1.2)));
        ...
        if (Math.abs(diffYaw) > 10.0f) {
            ...
            diffYaw *= NEwMathUtil.nextFloat(0.3, 0.7);
        }
        ...
        if (Math.abs(diffPitch) > 10.0f) {
            ...
            diffPitch *= NEwMathUtil.nextFloat(0.3, 0.7);
        }
        if (entity instanceof EntityLivingBase) {
            ...
            if (entityPlayer.hurtTime < 1) {
                ...
                if (smartAim && !throughWalls) {
                    ...
                    this.y = entityVec.yCoord - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight());
                ...
                if (Math.abs(diffY) > -1.0f) {
                    ...
                    diffY *= NEwMathUtil.nextFloat(0.3, 0.7);
                }
                ...
                if (Math.abs(diffP) > -1.0f) {
                    ...
                    diffP *= NEwMathUtil.nextFloat(0.3, 0.7);
                }
        ...
        if (yawSpeed > 0.5f && targetIsMoving) {
            ...
            bestYaw = (float)((NEwMathUtil.nextInt(0, 100000) % 2 == 0) ? (bestYaw - correctYaw) : (bestYaw + correctYaw));
        }
        while (bestPitch == perfectPitch) {
            bestPitch += (float)(NEwMathUtil.nextFloat(-1.0f, 1.0f) / 4.0f + NEwMathUtil.nextFloat(-1.0f, 1.0f) / 4.0f + MathHelper.clamp_float((float)ThreadLocalRandom.current().nextGaussian(), -1.0f, 1.0f) / 4.0f + NEwMathUtil.randomSin() / 4.0);
        }
        if (entity instanceof EntityLivingBase) {
            ...
            if (targetIsMoving && bestPitchRotationDistance == 0.0 && currentPitch != bestPitch) {
                ...
                if (suspiciousLevel > 8) {
                    ...
                    if (ACCURATE_ROTATION_PITCH_VL > 3.0) {
                        //mc.ingameGUI.getChatGUI().printChatMessage(new ChatComponentText("Too accurate pitch rotation §7(§c" + bestPitchRotationDistance + " §7| " + ACCURATE_ROTATION_PITCH_LEVEL + " §7| " + yawSpeed + ") "));
                    }
                }
            }
        }
